...
for (run_i in 1:num_runs) {
  ...
  #cross_similarities = similarities[grepl("base", rownames(similarities)), grepl("meta", colnames(similarities))]  # off-diagonal block of base x meta
  
  similarities[[run_i]] = similarities_matrix %>%
  #  upper.tri %>%
    as.data.frame %>%
    mutate(task1 = rownames(similarities_matrix)) %>%
#    filter(grepl("meta", task1)) %>%  # interested in cross-similarity, so restrict to have at least one meta to keep a manageable size
    pivot_longer(-task1, names_to="task2", values_to="similarity") %>%
 #   filter(grepl("base", task2)) %>% # restrict other to base
    mutate(run=run_i)
}
...
ggplot(data=similarities %>%
       ...
       aes(x=task1_is_meta, y=similarity, fill=task2_is_meta)) +
...
```{r}
ggplot(data.frame(variance=this_base_sds^2, rank=1:length(this_base_sds)),
...

```{r}
this_base_variances = this_base_sds^2
cumulative_base_variances = cumsum(this_base_variances / sum(this_base_variances))
cumulative_base_variances
```

```{r}
ggplot(data.frame(variance=this_meta_sds^2, rank=1:length(this_meta_sds)),
       aes(x=rank, y=variance)) +
  geom_bar(stat="identity")
```
...
cumulative_meta_variances = cumsum(this_meta_variances / sum(this_meta_variances))
cumulative_meta_variances
```

```{r}
image(PC_simils[1:10, 1:10], useRaster=T)
```
...
for (i in 1:num_sims) {
  ...
  dots[i] = abs(t(this_base_pcs[,1:1]) %*% x) # average abs similarity to top dimension
}
```

```{r}
ggplot(data=data.frame(avg_similarity=dots),
       aes(x=avg_similarity)) +
  geom_density()
```

What is the alpha=0.05 cutoff for significance under this distribution?

```{r}
cutoff = quantile(dots, c(0.975)) 
```

```{r}
ggplot(PC_simil_data %>%
         filter(base_PC <= 20),
       aes(x=meta_PC, y=base_PC, fill=abs(cosine_similarity))) +
  geom_raster() + 
  scale_fill_gradient(low="white", high="#008855", limits=c(0, 1))
```

```{r}

ggplot(control_PC_simil_data %>%
         filter(base_PC <= 20),
       aes(x=meta_PC, y=base_PC, fill=abs(cosine_similarity))) +
  geom_raster() + 
  scale_fill_gradient(low="white", high="#008855", limits=c(0, 1))
...
```{r}
ggplot(PC_simil_data %>%
         filter(base_PC <= 10, meta_PC < 5) %>%
         mutate(significant = abs(cosine_similarity) > cutoff),
       aes(x=base_PC, y=abs(cosine_similarity), fill=significant)) +
  facet_grid(meta_PC ~ .) +
  geom_bar(stat="identity") +
