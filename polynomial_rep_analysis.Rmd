...
#    scale_color_brewer(palette="Purples") +
  ...
  constant_poly_plots[[run_i]] = ggplot(pc_d %>%
    ...
    geom_point() +  
    scale_color_gradientn(colours=c("#a50026", "#d73027", "#f47848", "#f0a551", "#e0b060", "#505050", "#b0b8d0", "#99c4d4", "#74add1", "#4565b4", "#313695"),
...
```{r}
all_simil_d = data.frame()
for (run_i in 1:num_mapped_runs) {
  ...
  simil_d = similarities_matrix %>%
    ...
    mutate(run=run_i)
  all_simil_d = bind_rows(all_simil_d,
                          simil_d)
}
...
```{r}
ggplot(data=all_simil_d %>%
         mutate(task_base_or_meta=factor(task_base_or_meta, levels=c("base", "meta"), labels=c("Base", "Meta"))),
       aes(x=value, color=task_base_or_meta)) +
  facet_wrap(~ run) +
  geom_density() +
  scale_color_manual(values=c("#49006a", "#fa9fb5")) +
  geom_density()  +
  guides(color=guide_legend(title="Task type")) +
  labs(x="Cosine between input point and task representations", y="Density") +
  theme(legend.position=c(0.93, 0.28))

ggsave("../metamapping_paper/figures/polynomial_reps/input_to_task_rep_similarity.pdf", width=6, height=4)
```


```{r}
ggplot(data=all_simil_d %>%
         group_by(run, task_num_vars_relevant, point_num_vars_relevant) %>%
...
```{r}
all_simil_d %>%
  filter(task_base_or_meta == "base") %>%
  group_by(run) %>%
  do(res=summary(lm(.$value ~ .$task_X0 *as.numeric(.$X0) + .$task_X1 * as.numeric(.$X1) + .$task_X2 * as.numeric(.$X2) + .$task_X3 * as.numeric(.$X3)))) %>%
  pull(res) 
```
...
ggplot(data=all_simil_d %>%
         filter(task_base_or_meta == "base",
                !is.na(vars_relevant_IoU)),
       aes(x=as.factor(vars_relevant_IoU), color=as.factor(point_num_vars_relevant), y=value)) +
  facet_wrap(~run) +
  geom_violin()
```
...
```{r}
all_simil_d %>%
  filter(grepl("mult|add", task)) %>%
  mutate(
    mapping_type = str_extract(task, "add|mult"),
    mapping_scalar=as.numeric(str_extract(task, "[-]?[0-9]+\\.[0-9]+"))) %>%
  group_by(run, mapping_type) %>%
  do(res=summary(lm(.$value ~ .$mapping_scalar * (as.numeric(.$X0) + as.numeric(.$X1) + as.numeric(.$X2) + as.numeric(.$X3))))) %>%
  pull(res) 
```
