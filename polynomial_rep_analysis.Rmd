  ...
  ggplot(pc_d %>%
           ...
           mutate(constant_val = as.numeric(str_extract(task, "^[-]?[0-9]+\\.[0-9]++")),
                  compressed_constant_val = ifelse(constant_val > 0,
                                                   sqrt(constant_val),
                                                   -sqrt(-constant_val))), 
         aes(x=PC1_None, y=PC2_None, xend=`PC1_mapping_add_3.000000`, yend=`PC2_mapping_add_3.000000`, color=compressed_constant_val, shape=as.factor(num_vars_relevant))) +
    geom_segment(arrow=arrow()) +
    labs(x="PC 1", y="PC 2") +
    scale_color_gradient2() +
    guides(color=guide_colorbar(title="Constant\n(sqrt.)"),
           shape=guide_legend(title="Num. Rel.\nVariables"))
```
