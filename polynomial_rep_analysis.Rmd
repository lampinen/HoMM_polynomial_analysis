...
ggplot(square_match_plotting_d, 
       aes(x=source_PC1, y=source_PC2, xend=PC1, yend=PC2, color=compressed_constant_val, linetype=type)) +
  geom_segment(arrow=arrow(angle=15, length=unit(0.1, "inches"), type="closed")) +
  geom_text(data=square_match_plotting_d %>% filter(type == "Actual\noutput"),
            aes(label=task),
            nudge_y=-0.0005) +
  labs(x="PC 1", y="PC 2") +
  scale_color_gradient2(low="#a50026", mid="#eedd99", high="#313695",
                        breaks=c(-4, -2, 0, 2, 4),
                        labels=c(-16, -4, 0, 4, 16),
                        limits=c(-4, 4)) + 
...
mult_match_plotting_d = plotting_joined_d %>%
  filter(mapping == "mult_3",
         ...
         task %in% c("-3.55", "3.54X1", #"1.56+1.46X2", 
                     "-2.18+1.10X1", "3.97", "-1.60+-1.64X3", "0.43+0.34X0^2", "2.39+3.07X2+0.20X1^2+-2.23X2^2")) %>%
  mutate(type = factor(type, levels=c("target_output", "actual_output"), labels=c("Nominal\ntarget", "Actual\noutput")),
         task = task %>%
           gsub("X0", "w", .) %>%
           gsub("X1", "x", .) %>%
...
ggplot(mult_match_plotting_d, 
       aes(x=source_PC1, y=source_PC2, xend=PC1, yend=PC2, color=compressed_constant_val, linetype=type)) +
  geom_segment(arrow=arrow(angle=15, length=unit(0.1, "inches"), type="closed")) +
  geom_text(data=mult_match_plotting_d %>% filter(type == "Actual\noutput"),
            aes(label=task),
            nudge_y=-0.0005) +
  labs(x="PC 1", y="PC 2") +
  scale_color_gradient2(low="#a50026", mid="#eedd99", high="#313695",
