...
loss_d = bind_rows(guess_opt_loss_d, random_opt_loss_d, arbitrary_opt_loss_d)
...
ggplot(loss_d %>%
         filter(train_or_eval == "eval") %>%
         mutate(init_type=factor(init_type, levels=c("guess", "arbitrary_train_task", "random_init"), labels=c("Meta-mapping output", "Arbitrary train task", "Random vector"))),
       aes(x=epoch, y=log(loss), color=init_type)) +
  geom_line(stat="summary",
            fun.y="mean",
            na.rm=T,
  ...
  geom_line(aes(group=interaction(run, init_type)),
            ...
            na.rm=T,
            alpha=0.4) +
  geom_hline(yintercept=log(5.9), alpha=0.5, linetype=2) +
  scale_color_manual(values=c("#984ea3", "#477ec8", "#e41a1c")) +
  annotate("text", x=240, y=log(4.2), alpha=0.5, label="Chance") +
  labs(x="Epoch (training task embeddings on new data)", y="Log loss on new tasks") +
...
ggplot(loss_d %>%
         ...
         summarize(log_regret_int=log(sum(loss))) %>%
         group_by(init_type, run) %>%
         summarize(mean_lri=mean(log_regret_int, na.rm=T)) %>%
         ungroup() %>%
         mutate(init_type=factor(init_type, levels=c("guess", "arbitrary_train_task", "random_init"), labels=c("Meta-mapping output", "Arbitrary train task", "Random vector"))),
  ...
  geom_errorbar(stat="summary",
                ...
                width=0.5) +
  scale_fill_manual(values=c("#984ea3", "#477ec8", "#e41a1c")) +
  labs(x="Initialization", y="Mean log cumulative regret on new task") +
  guides(color=guide_legend(title=""))
